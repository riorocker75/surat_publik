...
function act_kk_tambah(){
	...
	$nama_masuk=$this->input->post('nama_masuk');
	...
	$surat_mohon="SKPnAKK-".$rand;
		...
		if($_FILES["ket_gk"]["name"]){
			$rand1=rand(1000,9999);
			$config['file_name'] = $rand1.'_'.$_FILES['ket_gk']['name'];				
			$this->load->library('upload', $config);
			$ket_gk = $this->upload->do_upload('ket_gk');
			if (!$ket_gk){
				$error = array('error' => $this->upload->display_errors());
			}else{
				$ket_gk = $this->upload->data("file_name");
				$data = array('upload_data' => $this->upload->data());
				$this->m_dah->update_data(array('id' => $id_terakhir),array('ket_gk' => $ket_gk),'sr_kk_tambah');			
			}
	...
	$surat_mohon="SKKTPB-".$rand;
		...
		$this->m_dah->insert_data($data_sm,'surat_mohon');
				...
				// end fotocopy KK
					...
					if($_FILES["ket_gk"]["name"]){
						$rand2=rand(10000,99999);
						$config['file_name'] = $rand2.'_'.$_FILES['ket_gk']['name'];				
						$this->upload->initialize($config); //wajib di inisiasi ini kalau untuk mulitple upload file
						...
						if (!$ket_gk){
							...
							$ket_gk = $this->upload->data("file_name");
							$data = array('upload_data' => $this->upload->data());
							$this->m_dah->update_data(array('id' => $id_terakhir),array('ket_gk' => $ket_gk),'sr_ktp_baru');			
							// redirect(base_url().'admin/sesi_surat/?alert=terkirim');
						}
...
*/					
			...
			if($_FILES["ket_gecik"]["name"]){
				$rand1=rand(1000,9999);
				$config['file_name'] = $rand1.'_'.$_FILES['ket_gecik']['name'];				
				$this->load->library('upload', $config);
				$ket_gecik = $this->upload->do_upload('ket_gecik');
				if (!$ket_gecik){
					$error = array('error' => $this->upload->display_errors());
				}else{
					$ket_gecik = $this->upload->data("file_name");
					$data = array('upload_data' => $this->upload->data());
					$this->m_dah->update_data(array('id' => $id_terakhir),array('ket_gk' => $ket_gecik),'sr_akte_lahir');			
				}
			
			}
		// end  keterangan gechick
...
*/					
		...
		if($_FILES["ket_gecik"]["name"]){
			...
			if (!$ket_gecik){
				...
				$ket_gecik = $this->upload->data("file_name");
				$data = array('upload_data' => $this->upload->data());
				$this->m_dah->update_data(array('id' => $id_terakhir),array('ket_gk' => $ket_gecik),'sr_akte_mati');			
			}
...
*/					
		...
		if($_FILES["ket_gecik"]["name"]){
			...
			if (!$ket_gecik){
				...
				$ket_gecik = $this->upload->data("file_name");
				$data = array('upload_data' => $this->upload->data());
				$this->m_dah->update_data(array('id' => $id_terakhir),array('ket_gk' => $ket_gecik),'sr_ket_pindah');			
			}
